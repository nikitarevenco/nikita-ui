...
import { toast, type ToastConfig } from "./toast-queue";
...
function ToastRegion({ state, ...props }: ToastRegionProps) {
  ...
  const position =
    state.visibleToasts.at(-1).content.position ?? "bottom-right";
...
function Toast({ state, ...props }: ToastProps) {
  ...
  const { type } = props.toast.content;
    ...
    >
      ...
      >
        ...
        >
          <XIcon
            aria-label="Close"
            className="text-muted/75 group-hover:text-foreground"
          />
        </Button>
      </div>
    </div>
